...
** Portable file and directory operations
   ...
   These functions provide portable file and directory operations 
   without using system() calls or fork/exec, for better portability
   and security.

*** C

   #+begin_src c :tangle prefix_front.h :exports none
trexio_exit_code trexio_rm(const char* path);
...
trexio_rm(const char* path)
{
  ...
  /* Try to determine the applicable backend if the back_end argument is TREXIO_AUTO */
  /* Check if the TREXIO file exists and if it is a directory */
  trexio_exit_code rc_text = trexio_text_inquire(path);
  if (rc_text == TREXIO_SUCCESS) {
    back_end_local = TREXIO_TEXT;
  }
#ifdef HAVE_HDF5
    ...
    /* If not, check if it is an HDF5 file */
    trexio_exit_code rc_hdf5 = trexio_hdf5_inquire(path);
    if (rc_hdf5 == TREXIO_SUCCESS) {
      back_end_local = TREXIO_HDF5;
    } else {
      /* File is neither a directory nor an HDF5 file -> return an error */
  ...
  if (back_end_local == TREXIO_TEXT) {
    ...
    return trexio_remove_directory_recursive(path);
  } else {
    /* File -> use file copy */
    return trexio_remove_file(path);
  }
}
   #+end_src
...
* General helper functions
  ...
  ~trexio_phase_aabb_to_abab~ and ~trexio_phase_aabb_to_abab_list~ are functions that help going from the $\alpha \alpha \ldots \beta\beta$
  ordering to the $\alpha \beta \alpha \beta \ldots$ ordering by checking if reordering a determinant requires applying a change of
  phase to the coefficient. These functions return ~TREXIO_SUCCESS~ if the conversion of the determinant doesn't require a change of
  phase. Otherwise, they return ~TREXIO_PHASE_CHANGE~.
